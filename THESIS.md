...
|---------|-------------|--------|
| Ch.1 S1 | Preamble sovereignty/identity rhetoric: scatter, balance bar, "indivisible" as heritage marker. Contextual analysis of "peoples" usage (61% francophone usage = Charter citations + diplomatic solidarity) | Delivered (M2) |
...
3. **Treaty ratification is structurally incapable of differentiating.** 5 of 6 treaties have near-universal ratification (89-98%); C169 has near-universal non-ratification (1/54). Treaty count variance is too low (std=0.6) for a correlation test to have power. Within Tier 1: Spearman ρ=−0.24, p=0.13 (n=42); across all 54: ρ=−0.06, p=0.68. The correct framing: the international regime gives every country the same score, making it structurally incapable of predicting constitutional diversity. This supports Ch.2 S1 more strongly than a null correlation alone.

4. **Recognition comes as a package.** Tier 1 (n=42): Drc ↔ Drm r=0.69, Dpa ↔ Drm r=0.68, Drm ↔ Id r=0.62. Countries either recognize the full identity cluster or stay silent. You cannot carve out "indigenous peoples" as a separate category — the functional criterion captures the whole package. Dc and Dis are near-universal and orthogonal to the identity cluster.

5. **Francophone silence is in the law, not in reality.** COMMENTAIRE analysis (same language, same coder — methodologically reliable) shows francophone entries use "ethni" at 13.7/1000 words vs anglophone 7.2. They *discuss* ethnicity more while *recognizing* it less constitutionally. Contextual analysis of "peoples" in constitution texts confirms: 61% of francophone usage = African Charter citations (24%) and diplomatic solidarity (37%), not domestic recognition. They cite the Charter on Human and *Peoples'* Rights without translating "peoples" into domestic law.
...
7. **Post-conflict constitutional moments are the strongest predictor — stronger than heritage.** Among Tier 1 countries (n=42), post-conflict constitutions score dramatically higher: mean 14.1 vs 7.5 (Mann-Whitney p=0.0001). This holds *within* each heritage group: francophone post-conflict 11.0 vs 6.8, anglophone post-conflict 15.7 vs 8.7. Heritage + post-conflict together explain 63.2% of variance (vs heritage alone 22.3%, conflict alone 51.0%). The effect is concentrated on the identity cluster: Dpa (p<0.001), Dau (p<0.001), Drc (p<0.001), Drm (p<0.001). The "outliers" (RDC, Kenya, South Africa, Ethiopia) are not heritage anomalies — they are post-conflict constitutions. This supports the thesis argument that recognition of sub-state peoples is driven by *political necessity* (the functional criterion), not by inherited legal tradition alone.

    *Post-conflict constitutional moment:* The **current constitution** (not the country as an inherent property) was adopted as a direct result of a negotiated peace process, post-war/post-genocide transition, or post-liberation state-building. Coups, authoritarian revisions, and routine amendments are excluded. 13/54 current constitutions coded as post-conflict (Sierra Leone and Liberia recoded to false — their constitutions predate their civil wars). Coding documented in `scripts/post_conflict_coding.json` with per-country justification.

8. **Semantic embeddings confirm heritage shapes form, not substance.** Legal-domain embeddings (voyage-law-2, 1024-dim) reveal a split: embedding-based clusters partially reproduce heritage groups (ARI=0.466) — anglophone constitutions are textually homogeneous, francophone+lusophone form another cluster. But dimension-based clusters (10 coded features) do NOT reproduce heritage (ARI=0.033). Heritage shaped constitutional *language and structure* (the colonial legal template persists in the text), but not *substantive recognition* (what rights are actually guaranteed). Substantive recognition is driven by post-conflict moments, not inherited form. Some geographic neighbors show high cross-heritage textual similarity (Rwanda↔Uganda sim=0.835, Mauritius↔Seychelles sim=0.861), suggesting proximity may matter as much as heritage for constitutional form.

9. **The ACHPR does not use the thesis's theoretical vocabulary.** Analysis of the case law corpus (94K chars, 3 docx files) reveals that "peuple infra-étatique", "critère fonctionnel", "identité complexe", and "protection systémique" do NOT appear in the Commission's decision texts. The Commission uses "peuples autochtones" (5 mentions) and "autodétermination" (2). The functional criterion is Alex's *interpretation* of what the ACHPR does, not the ACHPR's own language. This is methodologically important: the thesis constructs the category, the Commission applies it without naming it.

10. **Francophone silence is on identity, not culture.** Extended coding (12 sub-categories across land, resources, environment, cultural rights) reveals NO significant heritage differences on cultural rights (cultural heritage, cultural practices, cultural institutions: all p>0.10, Tier 1 Mann-Whitney). The francophone silence is specifically on *identity recognition* (Drm p<0.001, Id p<0.001) — not on cultural provisions broadly. Francophone constitutions even lead on environmental language (p=0.003) and listing official languages. The silence is surgical: it applies to the recognition of *groups*, not to *culture* in general. Computed from `data/extended_coding_v2.json` in `notebooks/eda_constitutional_dataset.ipynb`.

11. **Religion is not an independent factor.** Constitutional references to Islam, secularism, or religious neutrality do not predict recognition scores independently of colonial heritage (η² increment = 2.0 percentage points, Kruskal-Wallis p=0.45). Within each heritage group, religion makes no difference: francophone Islamic countries score 6.7 ≈ francophone secular 7.6; anglophone Islamic 9.3 ≈ anglophone neutral 11.2. The religion signal is fully absorbed by heritage. Religion coding in `scripts/religion_coding.json`, computation in `notebooks/eda_constitutional_dataset.ipynb`.
