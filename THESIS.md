...
8. **Semantic embeddings confirm heritage shapes form, not substance.** Legal-domain embeddings (voyage-law-2, 1024-dim) reveal a split: embedding-based clusters partially reproduce heritage groups (ARI=0.466) — anglophone constitutions are textually homogeneous, francophone+lusophone form another cluster. But dimension-based clusters (10 coded features) do NOT reproduce heritage (ARI=0.033). Heritage shaped constitutional *language and structure* (the colonial legal template persists in the text), but not *substantive recognition* (what rights are actually guaranteed). Substantive recognition is driven by post-conflict moments, not inherited form. Geographic neighbors are more textually similar than heritage peers (Rwanda↔Uganda sim=0.835, Mauritius↔Seychelles sim=0.861).

9. **The ACHPR does not use the thesis's theoretical vocabulary.** Analysis of the case law corpus (94K chars, 3 docx files) reveals that "peuple infra-étatique", "critère fonctionnel", "identité complexe", and "protection systémique" do NOT appear in the Commission's decision texts. The Commission uses "peuples autochtones" (5 mentions) and "autodétermination" (2). The functional criterion is Victor's *interpretation* of what the ACHPR does, not the ACHPR's own language. This is methodologically important: the thesis constructs the category, the Commission applies it without naming it.

10. **Francophone silence is on identity, not culture.** Extended coding (12 sub-categories across land, resources, environment, cultural rights) reveals NO significant heritage differences on cultural rights (cultural heritage, cultural practices, cultural institutions: all p>0.10). The francophone silence is specifically on *identity recognition* (Drm p<0.001, Id p<0.001) — not on cultural provisions broadly. Francophone constitutions even lead on environmental language (p=0.003) and listing official languages. The silence is surgical: it applies to the recognition of *groups*, not to *culture* in general.
